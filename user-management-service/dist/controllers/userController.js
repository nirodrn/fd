...
export const registerUser = async (req, res) => {
  try {
    ...
    // Send welcome email
    try {
      await sendEmail(email, "Welcome to Foodie App", `Hi ${name}, your account has been created successfully.`);
    } catch (emailError) {
  ...
  try {
    ...
    if (user.role !== expectedRole) {
      return res.status(403).json({
        message: `Access denied. You must be a ${expectedRole}.`
      });
    }
...
export const updateUserProfile = async (req, res) => {
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(req.user.userId, updateFields, {
      new: true
    }).select("-password");

    // Send profile update notification email
    await sendEmail(updatedUser.email, "Profile Updated Successfully", `Dear ${updatedUser.name},\n\nYour profile was successfully updated on Foodie App.\n\nBest regards,\nFoodie App Team`);
...
export const deleteUser = async (req, res) => {
  try {
    ...
    await sendEmail(user.email, "Account Deleted", `Dear ${user.name},\n\nYour account has been deleted from Foodie App.\n\nIf you have any questions, please contact support.\n\nBest regards,\nFoodie App Team`);
